...
eyes.inspect = function (obj, label, options) {
    ...
    if (options.stream) {
        ...
        return stringify(obj, options) + (options.styles ? '\x0f' : '');
    }
};
...
eyes.stylize = function (str, style, styles) {
    ...
    if (styles && style && codes[style]) {
        //endCode = (codes[style][1] === 39 && styles.all) ? codes[styles.all][0] : codes[style][1];
        return (style == 'bold' ? '\x02' : '\x03' + codes[style]) + str + '\x0f';
    } else { return str }
};
...
function typeOf(value) {
    var s = typeof(value),
        types = [Array, String, RegExp, Number, Function, Boolean, Date, Object];

    if (s === 'object' || s === 'function') {
        if (value) {
            types.some(function (t) {
                if (value.constructor.toString() == t.toString()) { s = t.name.toLowerCase(); return true; }
            });
        } else { s = 'null' }
    }
    return s;
